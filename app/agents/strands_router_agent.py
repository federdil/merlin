...
class RoutingDecision(BaseModel):
    ...
    agent_type: str = Field(description="The agent type to route to: 'ingestion', 'query', 'summarization', 'knowledge_gap', 'conversational_query', or 'learning_path'")
...
- **knowledge_gap**: For identifying knowledge gaps and learning opportunities
- **conversational_query**: For natural language queries with temporal awareness
- **learning_path**: For generating personalized learning paths and recommendations

Routing rules:
1. URLs (http/https) → ingestion agent with action "ingest_url"
2. Questions with temporal references ("last month", "recently", "when I started") → conversational_query agent with action "process_conversational_query"
3. Questions starting with "what", "how", "where", "when", "who", "find", "search" → query agent with action "search"
4. Text starting with "summarize", "summary", "brief", "overview" → summarization agent with action "summarize_existing"
5. Queries about "gaps", "missing", "what should I learn", "knowledge gaps" → knowledge_gap agent with action "detect_gaps"
6. Queries about "learning path", "what to study next", "roadmap", "learning plan" → learning_path agent with action "suggest_learning_path"
7. Long text content (paragraphs) → ingestion agent with action "ingest_text"
8. Empty or minimal input → query agent with action "empty_input"
...
"""
    ...
    def _prepare_input_data(self, agent_type: str, action: str, original_input: str, 
        ...
        if agent_type == 'ingestion':
            ...
            return {
                **base_data,
                'user_id': 'default_user',  # In a real system, get from context
                'timeframe': '30d'
            }
            ...
            return {
                **base_data,
                'query': content or original_input,
                'user_id': 'default_user',  # In a real system, get from context
                'session_id': None  # Will be generated by the agent
            }
            ...
            return {
                **base_data,
                'user_id': 'default_user',  # In a real system, get from context
                'topic': None,  # Will be extracted from query
                'current_level': None,  # Will be assessed by the agent
                'objectives': None  # Will be generated by the agent
            }
    ...
    def _fallback_routing(self, user_input: str, title: Optional[str], 
        ...
        if input_type == 'url':
            ...
            # Check for temporal references first
            if any(word in text_lower for word in ['last month', 'recently', 'when i started', 'this year', 'yesterday', 'today']):
                return {
                    'agent_type': 'conversational_query',
                    'action': 'process_conversational_query',
                    'input_data': {
                    ...
                    'confidence': 0.9,
                    'reasoning': 'Temporal references detected - routing to conversational query'
                }
            
            # Check for knowledge gap keywords
            elif any(word in text_lower for word in ['gaps', 'missing', 'what should i learn', 'knowledge gaps', 'what am i missing']):
                return {
                    'agent_type': 'knowledge_gap',
                    'action': 'detect_gaps',
                    'input_data': {
                        'user_id': 'default_user',
                        'timeframe': '30d',
                    ...
                    'confidence': 0.9,
                    'reasoning': 'Knowledge gap keywords detected'
                }
            
            # Check for learning path keywords
            elif any(word in text_lower for word in ['learning path', 'what to study next', 'roadmap', 'learning plan', 'study plan']):
                return {
                    ...
                    'confidence': 0.9,
                    'reasoning': 'Learning path keywords detected'
                }
            
            # Check for summarization keywords
            elif any(word in text_lower for word in ['summarize', 'summary', 'brief', 'overview']):
                return {
    ...
    def get_capabilities(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'supported_agents': ['ingestion', 'query', 'summarization', 'knowledge_gap', 'conversational_query', 'learning_path'],
    ...
    def validate_routing(self, routing_result: Dict[str, Any]) -> bool:
        ...
        valid_agents = ['ingestion', 'query', 'summarization', 'knowledge_gap', 'conversational_query', 'learning_path']
